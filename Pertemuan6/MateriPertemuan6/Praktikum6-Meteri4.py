...
def merge_sort(data):
    indent = ' ' * 4 * (len(data) - 1)  #indentasi untuk visualisasi proses merge sort
    print(f"{indent}merge_sort({data})")  #menampilkan proses merge sort
    ...
    print(f"{indent}divide: left={left}, right={right}")  #menampilkan proses pembagian data
    
    #recursive call untuk membagi data hingga mencapai base case (data dengan satu elemen atau kosong)
    left = merge_sort(left)
    right = merge_sort(right)
    print(f"{indent}merge({left}, {right})")  #menampilkan proses merge sort
